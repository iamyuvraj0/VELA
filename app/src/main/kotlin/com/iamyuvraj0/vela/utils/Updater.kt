...
import com.iamyuvraj0.vela.App
import com.iamyuvraj0.vela.constants.GitHubReleasesEtagKey
import com.iamyuvraj0.vela.constants.GitHubReleasesFingerprintKey
import com.iamyuvraj0.vela.constants.GitHubReleasesJsonKey
import com.iamyuvraj0.vela.constants.GitHubReleasesLastCheckedAtKey
import com.iamyuvraj0.vela.constants.UpdateChannel
import com.iamyuvraj0.vela.constants.UpdateChannelKey
...
private const val VELA_REPOSITORY = "iamyuvraj0/VELA"
private const val USER_AGENT = "VELA"
private const val STABLE_APK_ASSET_NAME = "VELA.apk"
private const val NIGHTLY_TAG_NAME = "nightly"
private const val NIGHTLY_APK_ASSET_NAME = "VELA.apk"
...
object Updater {
    ...
    private suspend fun fetchReleasesNetwork(
        ...
        val response: HttpResponse =
            client.get("https://api.github.com/repos/$VELA_REPOSITORY/releases?per_page=$perPage") {
                headers {
    ...
    private suspend fun fetchNightlyJson(): Result<NightlyInfo> = runCatching {
        val response = client.get("https://api.github.com/repos/$VELA_REPOSITORY/releases/tags/$NIGHTLY_TAG_NAME") {
            headers {
                append("Accept", "application/vnd.github+json")
                append("User-Agent", USER_AGENT)
            }
        ...
        if (apkUrl.isBlank()) {
            apkUrl = "https://github.com/$VELA_REPOSITORY/releases/download/$NIGHTLY_TAG_NAME/$NIGHTLY_APK_ASSET_NAME"
        }
     ...
     * de `VELA.apk`. Si la llamada falla o el asset no está listado,
     * usa la URL canónica de GitHub como fallback.
     */
    private suspend fun resolveApkDownloadUrl(tagName: String): String {
        val fallback =
            "https://github.com/$VELA_REPOSITORY/releases/download/$tagName/$STABLE_APK_ASSET_NAME"

        return runCatching {
            val response = client.get(
                "https://api.github.com/repos/$VELA_REPOSITORY/releases/tags/$tagName"
            ) {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", USER_AGENT)
    ...
    suspend fun getCommitHistory(count: Int = 20, branch: String = "master"): Result<List<GitCommit>> =
        runCatching {
            val response =
                client.get("https://api.github.com/repos/$VELA_REPOSITORY/commits?sha=$branch&per_page=$count") {
                    headers {
                        append("Accept", "application/vnd.github+json")
                        append("User-Agent", USER_AGENT)
                    }
    ...
    fun getLatestDownloadUrl(): String {
        ...
        return when (channel) {
            UpdateChannel.STABLE -> {
                "https://github.com/$VELA_REPOSITORY/releases/latest/download/$STABLE_APK_ASSET_NAME"
            }
            UpdateChannel.NIGHTLY -> {
                cachedNightlyInfo?.apkUrl
                    ?: "https://github.com/$VELA_REPOSITORY/releases/download/$NIGHTLY_TAG_NAME/$NIGHTLY_APK_ASSET_NAME"
            }
        }
    }
